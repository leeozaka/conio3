...
void gotoxy(int x, int y) {
    ...
    coord.X = __CONIO_LEFT + x - 1;
    coord.Y = __CONIO_TOP + y - 1;
...
void highvideo(void) {
    if (__FOREGROUND < CINZA_ESCURO) textcolor(__FOREGROUND + 8);
}

void lowvideo(void) {
    if (__FOREGROUND > CINZA_CLARO) textcolor(__FOREGROUND - 8);
}
